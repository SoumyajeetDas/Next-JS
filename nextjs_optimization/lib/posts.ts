import { PostType } from '@/app/new-post/page';
...
export async function getPosts(maxNumber: number): Promise<PostType[]> {
  ...
  return maxNumber
    ? (stmt.all(maxNumber) as PostType[])
    : (stmt.all() as PostType[]);
}
...
export async function updatePostLikeStatus(postId: string, userId: number) {
  ...
  stmt.get(userId, postId);
  const isLiked = (stmt.get(userId, postId) as any).count === 0;
